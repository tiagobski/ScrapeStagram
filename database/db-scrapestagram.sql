...
CREATE TABLE IF NOT EXISTS `scrapestagram`.`accounts` (
  ...
  `external_id_accounts` BIGINT NOT NULL,
...
-- -----------------------------------------------------
-- Table `scrapestagram`.`posts_has_images`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `scrapestagram`.`posts_has_images` (
  `ref_id_posts` INT NOT NULL,
  `ref_id_images` INT NOT NULL,
  PRIMARY KEY (`ref_id_posts`, `ref_id_images`),
  INDEX `fk_posts_has_images_images1_idx` (`ref_id_images` ASC),
  INDEX `fk_posts_has_images_posts1_idx` (`ref_id_posts` ASC),
  CONSTRAINT `fk_posts_has_images_posts1`
    FOREIGN KEY (`ref_id_posts`)
    REFERENCES `scrapestagram`.`posts` (`id_posts`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_posts_has_images_images1`
    FOREIGN KEY (`ref_id_images`)
